...
async function main() {
  ...
  // Real price row
  let price = await prisma.fuelPrice.findFirst({ where: { fuelKind: "AUTO_DIESEL", pricePerLitre: PRICE_CENTS }, orderBy: { effectiveFrom: "desc" } });
  if (!price) price = await prisma.fuelPrice.create({ data: { fuelKind: "AUTO_DIESEL", pricePerLitre: PRICE_CENTS, effectiveFrom: dayDate(2026, 3, 1), source: "MANUAL", note: "Badalgama actual Rs 277/L", enteredById: sysId } });
    ...
    for (const row of raw.slice(6)) {
      ...
      if (!asset) {
        ...
        const created = await prisma.asset.create({
          data: { code, typeLabel: type, categoryId: cat.id, meterType: cat.defaultMeterType, status: "ACTIVE", projectId: badal.id },
        });
        ...
        // Matched. If unassigned, attach to Badalgama; if already on a site, leave it.
        const full = await prisma.asset.findUnique({ where: { id: asset.id }, select: { projectId: true } });
        if (!full?.projectId) { await prisma.asset.update({ where: { id: asset.id }, data: { projectId: badal.id } }); stats.toBadal++; }
        else stats.toSite++;
      }
