...
async function main() {
  ...
  // AUTO_DIESEL price (Rs 350/L placeholder; Ceypetco site blocks automation)
  ...
  if (!price) {
    price = await prisma.fuelPrice.create({
      data: { fuelKind: "AUTO_DIESEL", pricePerLitre: 35000, effectiveFrom: WINDOW_START, source: "MANUAL", note: "Placeholder for CEP running import", enteredById: sysUser.id },
    });
  }
  ...
  for (const file of FILES) {
    ...
    for (const sheetName of wb.SheetNames) {
      ...
      for (const row of rows.slice(11)) {
        ...
        const start = toFloat(r[2]); const end = toFloat(r[4]);
        const distOrHrs = toFloat(r[6]); const hoursWkd = toFloat(r[9]);
        const litres = toFloat(r[11]);
        const working = distOrHrs > 0 || hoursWkd > 0 || litres > 0 || (start > 0 && end > start);
  ...
  stats.delF = (await prisma.fuelIssue.deleteMany({ where: { assetId: { in: ids }, issueDate: { gte: WINDOW_START, lt: WINDOW_END } } })).count;
